...
public final class De60Parser implements FieldValueDecoder {
  ...
  private Subfield parseSubfield(BcdSubfield subfield) {
    if (subfield.id().equals("12")) {
      return new Subfield(
          "12", "Dados de Criptografia da Trilha", parseTrackEncryption(subfield.body()));
    }
    return new Subfield(
  ...
  private TrackEncryptionData parseTrackEncryption(byte[] body) {
    if (body.length < 14) {
      throw new IsoParseException(
          "DE060.12 incompleto: esperados 14 bytes, recebidos %d".formatted(body.length));
    }
  ...
  public record TrackEncryptionData(String cardBin, EncryptionType encryptionType, String ksn) {}
