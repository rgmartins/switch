...
public record PosMessage(
  ...
  public String toMultilineString() {
    ...
    output.append(
        "TPDU   id=%s destino=%s origem=%s%n"
  ...
  private void appendField(StringBuilder output, int de, ParsedField field) {
    ...
    if (!value.contains(System.lineSeparator())) {
      output.append(
          "DE%03d %-24s = %s  %s%n".formatted(de, field.definition().key(), value, metadata));
      return;
    }

    output.append("DE%03d %s %s =%n".formatted(de, field.definition().key(), metadata));
