...
public final class De12Parser implements FieldValueDecoder {
  ...
  public Object decode(byte[] raw, FieldDef definition) {
    ...
    return new TransactionDateTime(
        value,
        "%s-%s-%s".formatted(value.substring(0, 2), value.substring(2, 4), value.substring(4, 6)),
        "%s:%s:%s"
            .formatted(value.substring(6, 8), value.substring(8, 10), value.substring(10, 12)));
  }

  public record TransactionDateTime(String raw, String date, String time) {
