...
public final class De47Parser implements FieldValueDecoder {
  ...
  private Subfield parseSubfield(BcdSubfield subfield) {
    return switch (subfield.id()) {
      ...
      case "02" ->
          new Subfield(
              "02",
              "Dados Estatísticos de Suprimentos da Transação Anterior",
              parse02(subfield.body()));
  ...
  private ConnectionStatistics parse01(byte[] body) {
    ...
    while (cursor.hasRemaining()) {
      int length =
          BcdCodec.decodeLength(
              cursor.readBytes(2, "DE047.01.ocorrencia.tamanho"), "DE047.01.ocorrencia");
      occurrences.add(parseOccurrence(length, cursor.readBytes(length, "DE047.01.ocorrencia")));
    }
  ...
  private SupplyStatistics parse02(byte[] body) {
    ...
    return new SupplyStatistics(bcdInt(body, 0, 1), bcd(body, 1, 3), bcdInt(body, 4, 1));
  }
  ...
  /**
   * Compatibilidade isolada com o contador de linhas do protocolo legado, que pode trazer nibbles
   * acima de 9 (ex.: 4E20 -> "1420").
   */
  ...
  private String printableAscii(byte[] source) {
    return new String(source, StandardCharsets.US_ASCII).replaceAll("[^\\x20-\\x7E]", "").strip();
  }
