...
/**
 * Referência: pos-mapper.service.ts (guzula-switch), método buildCanonico.
 * Converte a {@link PosTransaction} (visão tipada da captura POS) para o modelo canônico.
 * Todo campo do canônico é montado exclusivamente a partir de PosTransaction — nunca
 * lendo a {@link PosMessage} ou o payload bruto diretamente.
 *
 * TODO: DE061.03 (número de série do pinpad) e DE061.09 (transação original, usada no
 * cancelamento) ainda não têm subcampo decodificado em De61Parser; equipment.numberSeriePinpad
 * e originalTransaction ficam em branco até isso ser portado.
 * TODO: portar interacoes/registros/time, que dependem do dump da mensagem bruta e de
 * marcações de tempo hoje não disponíveis nesta camada.
 */
@Service
public class PosMapperService {
    ...
    private String trackEncryptionKsn(PosTransaction transaction) {
        ...
        if (subfield12 != null && subfield12.details() instanceof De60Parser.TrackEncryptionData details) {
            return details.ksn();
        }
        return null;
    }
    ...
    private CanonicalTransaction.Merchant merchant(PosTransaction transaction) {
        String merchantId = transaction.de042MerchantId() == null ? "" : transaction.de042MerchantId().toString();
        String merchantLocation = transaction.de043MerchantLocation() == null ? "" : transaction.de043MerchantLocation().toString();
    ...
    private CanonicalTransaction.Equipment equipment(PosTransaction transaction) {
        ...
        equipment.setDateTimeLocal(transaction.de012TimeLocalTransaction() == null ? "" : transaction.de012TimeLocalTransaction().raw());
    ...
    private CanonicalTransaction.Equipment.PinPad pinPad(String pinpadFisico) {
        ...
        pinPad.setHasPinpadChip(Set.of(
    ...
    private CanonicalTransaction.Card card(PosTransaction transaction) {
        ...
        boolean isContactless = SwitchConstants.EntryMode.CONTACTLESS_TRACK.equals(formaEntrada)
    ...
    private String inputMethod(String formaEntrada) {
        if (SwitchConstants.EntryMode.CHIP.equals(formaEntrada) || SwitchConstants.EntryMode.CONTACTLESS_CHIP.equals(formaEntrada)) {
            return SwitchConstants.InputMethod.CHIP;
        }
        if (SwitchConstants.EntryMode.TRACK_2.equals(formaEntrada) || SwitchConstants.EntryMode.CONTACTLESS_TRACK.equals(formaEntrada)) {
    ...
    private CanonicalTransaction.Operation operation(PosTransaction transaction) {
        ...
        String processingCode = transaction.de003ProcessingCode() == null ? "" : transaction.de003ProcessingCode();
        String mti = transaction.mti() == null ? "" : transaction.mti();

        boolean isDebitFlow = isDebitFlow(hostFlow);
        boolean isCreditSimulation = PosConstants.ProcessingCode.NAO_SEI_650000.equals(processingCode)
        ...
        operation.setTransactionModality(isDebitFlow ? SwitchConstants.Trn.MODALITY_DEBIT : SwitchConstants.Trn.MODALITY_CREDIT);
        ...
        operation.setDateTime(transaction.de012TimeLocalTransaction() == null ? "" : transaction.de012TimeLocalTransaction().raw());
    ...
    private boolean isDebitFlow(int hostFlow) {
        boolean isKnownDebitFlow = hostFlow == PosConstants.HostFlow.SOLUTION_SALE_AND_VISA_VALE_4
    ...
    private CanonicalTransaction.Operation.InstallmentPlan installmentPlan(PosTransaction transaction) {
        ...
        CanonicalTransaction.Operation.InstallmentPlan installmentPlan = new CanonicalTransaction.Operation.InstallmentPlan();
    ...
    private CanonicalTransaction.Operation.Product product(PosTransaction transaction) {
        ...
        CanonicalTransaction.Operation.Product.UniqueProduct uniqueProduct = new CanonicalTransaction.Operation.Product.UniqueProduct();
        uniqueProduct.setProduct(matrixProduct);
        uniqueProduct.setProductSub(secondaryProduct);

        CanonicalTransaction.Operation.Product.Settlement settlement = new CanonicalTransaction.Operation.Product.Settlement();
    ...
    private CanonicalTransaction.Operation.Cdc cdc(int hostFlow, String processingCode) {
        ...
        cdc.setIsCDCConsultation(hostFlow == PosConstants.HostFlow.CDC_QUERY || hostFlow == PosConstants.HostFlow.BALANCE_QUERY);
        cdc.setIsCDCConsultationDebit(PosConstants.ProcessingCode.CDC_CONSULTING_DEBIT.equals(processingCode));
        return cdc;
    }

    private CanonicalTransaction.Operation.PrivateData privateData() {
        CanonicalTransaction.Operation.PrivateData privateData = new CanonicalTransaction.Operation.PrivateData();
    ...
    private CanonicalTransaction.ConfirmacaoEnviada confirmacaoEnviada(PosTransaction transaction) {
        ...
        CanonicalTransaction.ConfirmacaoEnviada confirmacaoEnviada = new CanonicalTransaction.ConfirmacaoEnviada();
    ...
    private List<Anotacao> anotacoes(PosTransaction transaction) {
        ...
        if (sub08 != null && sub08.details() instanceof De61Parser.FinancingData details) {
            ...
            boolean isZeroed = isBlankOrZero(financingCode, "00")
