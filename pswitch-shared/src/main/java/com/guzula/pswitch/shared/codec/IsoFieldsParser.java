...
public final class IsoFieldsParser {
    ...
    public IsoFieldsParser(
            Map<Integer, FieldDef> schema,
            Map<Integer, FieldValueDecoder> customDecoders) {
    ...
    public Map<Integer, ParsedField> parse(ByteCursor cursor, IsoBitmap bitmap) {
        ...
        for (int de = 2; de <= bitmap.bitCount(); de++) {
            ...
            FieldValueDecoder decoder = customDecoders.getOrDefault(
                    de,
                    (value, field) -> IsoCodec.parseIsoField(value, field.type(), field.length()));
    ...
    private int readLength(ByteCursor cursor, FieldDef definition) {
        ...
        byte[] prefix = cursor.readBytes(
                definition.format().prefixBytes(),
                "tamanho do DE %03d".formatted(definition.de()));
