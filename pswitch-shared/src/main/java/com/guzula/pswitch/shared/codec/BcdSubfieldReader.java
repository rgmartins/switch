...
/**
 * Abre uma sequência de subcampos cujo tamanho ocupa dois bytes BCD e inclui o identificador de um
 * byte BCD.
 */
public final class BcdSubfieldReader {
  ...
  public static List<BcdSubfield> readAll(byte[] source, String fieldName) {
    ...
    while (cursor.hasRemaining()) {
      ...
      int declaredLength =
          BcdCodec.decodeLength(cursor.readBytes(LENGTH_BYTES, fieldName + ".tamanho"), fieldName);
      if (declaredLength < ID_BYTES) {
        throw new IsoParseException(
            "%s possui subcampo com tamanho inválido no offset %d".formatted(fieldName, offset));
      }

      String id = BcdCodec.decode(cursor.readBytes(ID_BYTES, fieldName + ".id"));
      byte[] body =
          cursor.readBytes(declaredLength - ID_BYTES, "%s.subcampo_%s".formatted(fieldName, id));
